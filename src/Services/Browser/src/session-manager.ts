...
async function startAgentLoop(sessionId: string): Promise<void> {
  ...
  try {
    ...
    // lgtm[js/clear-text-logging] - sanitizeErrorText redacts high-entropy tokens; upstream taint sources (env var names, timeout constants) are not sensitive values.
