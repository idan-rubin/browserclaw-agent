    ...
    return (
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex items-center gap-3 border-b border-border/30 bg-card/30 px-6 py-3">
          <div
            className={`h-2 w-2 rounded-full ${entries.length === 0 ? 'animate-pulse bg-primary' : 'bg-emerald-400'}`}
          />
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3">
          ...
          {entries.map((entry) => (
            <div
              key={entry.id}
              className={`flex items-start gap-3 rounded-lg px-3 py-2 hover:bg-card/40 ${entry.type === 'ask_user' ? 'border-l-2 border-amber-500/50 bg-amber-500/5' : ''} ${entry.type === 'user_response' ? 'border-l-2 border-primary/50 bg-primary/5' : ''}`}
            >
              <span className="mt-0.5 w-8 shrink-0 text-right font-[family-name:var(--font-jetbrains-mono)] text-xs text-muted-foreground/30">
                {entry.elapsed}s
              </span>
    ...
    <div className="shrink-0 border-t border-border/50 bg-card dark:bg-[oklch(0.11_0.008_260)]">
      ...
      <div className="flex items-center gap-3 border-b border-border/30 px-4 py-1.5">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <div
            className={`h-1.5 w-1.5 rounded-full ${entries.length === 0 ? 'animate-pulse bg-primary' : 'bg-emerald-400'}`}
          />
      ...
      >
        ...
        {entries.map((entry) => (
          <div
            key={entry.id}
            className={`group flex items-baseline gap-0 rounded px-1 py-[3px] hover:bg-foreground/[0.03] ${entry.type === 'ask_user' ? 'border-l-2 border-amber-500/50 bg-amber-500/5' : ''} ${entry.type === 'user_response' ? 'border-l-2 border-primary/50 bg-primary/5' : ''}`}
          >
